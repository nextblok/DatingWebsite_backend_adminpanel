      ...
      try {
        ...
        if (response.data.success) {
          ...
          let newdata = data.map((prop, key) => {
            return {
              ...prop,
              buysell: prop.buysell == 'buy' ? <span style={{ color: 'green' }}>Buy</span> : <span style={{ color: 'red' }}>Sell</span>,
              actions: (
      ...
      <div className="content">
        <Card>
          ...
          <CardBody>
            <h6>Balance</h6>
            <Row>
              {Object.keys(wallet).map((coin, index) =>
                <Col md={3}>
                  <span>{coin}: {wallet[coin].balance}</span>
                </Col>
              )}
            </Row>
            <hr />
